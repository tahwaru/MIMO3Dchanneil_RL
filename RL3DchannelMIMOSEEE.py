...
#initial state
Pjmax=Ptot/Nlink
IndexS=[0,0,0] # Index of the state Ptx, Tilt and nt values at t
...
for j in range(Nlink):
    ...
    print("tilt=",Tilt[0])
    print("        ")
    SE[j,0]=capa(Nr,nt[0],N,distD2D,PTx[0], Tilt[0])
    SE_RL[j,0]=SE[j,0]
    n_sum=1/np.linspace(2,nt[0]+1,nt[0]+1) # Generation of n_sum components
    mu=0
    for l in range(nt[0]):
        mu=mu + 1/n_sum[l]
        
    EE[j,0]=W*SE[j,0]/(2*(1e6)*(mu*PTx[0]/nt[0] + nt[0]*Pcirc))
    EE_RL[j,0]=EE[j,0]
    TPC[j,0]=mu*PTx[0]/nt[0]+nt[0]*Pcirc
    TPC_RL[j,0]=TPC[j,0]
OutageIndic=0  # Indicator of SE outage
TiltSE=np.zeros(Tilt.shape[0])
for t in range(1,Ncg):    
    for j in range(1,Nlink): # We can average over the number of Ncg
