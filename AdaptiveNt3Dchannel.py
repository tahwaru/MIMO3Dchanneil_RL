# -*- coding: utf-8 -*-
"""
Created on Thu Jul 23 23:17:43 2020
...
Tilt=np.pi/np.linspace(20,80,100) 
...
for iter in range(ITER):
    Nt=Nt + 2 
    Nr=Nt
    Pa=P_BS/(Pcirc*Nt**2)  # amplifier power
    Ntparam.append(np.sqrt(Pa*P_BS/Pcirc))    
    for chan in range(NumChan):
        SE[iter]=SE[iter] + capa(Nr,Nt,N,distD2D[iter],P_BS)
        
    SE[iter]=SE[iter]/NumChan
    
    EE[iter]=W*SE[iter]/(2*(1e6)*(Pa*P_BS/Nt + Nt*Pcirc)) # Non optimal Energy efficiency    
...
plt.plot(distD2D,SE, marker='^',label="Optimal SE_j, n_t=16")
...
plt.plot(SE,EE, marker='*',label="Non-Optimal EE_j, n_t=16")
plt.plot(SE,EEopt2, marker='+',label="Optimal EE_j, n_t=16")
plt.legend(loc='upper left')
#plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0.)
