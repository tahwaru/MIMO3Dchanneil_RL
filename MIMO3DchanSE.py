...
Nt=4  # Number of transmit antenna ports
Nr=4  # Number of receive antenna ports
...
N=3  # Number of paths in a multipath propagation
...
def capa(nr,nt,n,d,Ptx):
    ...
    for jj in range (NumInt):
      ...
      #---------
    No=W * noiseFigure * N0
    myu =np.array([0.08, 0.12, 0.4, 0.4]) #Probability of the realization of each channel; 
    tempSum=0 
...
for iter in range(Pt.shape[0]):
    
    Ntparam.append(np.sqrt(mu*Pt[iter]/Pcirc))    
    for chan in range(NumChan):
        SE[iter]=SE[iter] + capa(Nr,Nt,N,Tx_Rx_dist,Pt[iter])
        
    SE[iter]=SE[iter]/NumChan
    
    EE[iter]=W*SE[iter]/(2*(1e6)*(mu*Pt[iter]/Nt + Nt*Pcirc)) # Non optimal Energy efficiency    
    #EEopt[iter]=W*SE[iter]/(2*(1e6)*(mu*Pt[iter]/Ntparam[iter]+mu*Ntparam[iter])) #Optimal energy efficiency; # dividing by 1e6 to present EE in Mbps/Joule 
    EEopt2[iter]=np.sqrt(mu*Pt[iter]/Pcirc)*W*SE[iter]/(4*(1e6)*(Pt[iter]*mu))
#Optimality 
#Aval=Pcirc*n_sum
#Bval=Pt/(Nt**2)
...
plt.plot(Pt,SE, marker='^',label="Optimal SE_j, n_t=8")
...
#plt.plot(SE,EE, marker='*',label="Non-Optimal EE_j, n_t=8")
#plt.plot(SE,EEopt, marker='+',label="Optimal EE_j, n_t=8")
...
plt.plot(SE,EE, marker='*',label="Non-Optimal EE_j, n_t=8")
plt.plot(SE,EEopt2, marker='+',label="Optimal EE_j, n_t=8")
plt.legend(loc='upper left')
#plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0.)
